...
const SYSTEM_PROMPT = `أنت مساعد قانوني خبير في القانون المصري، تعمل داخل منصة عامة لإدارة المكاتب القانونية وخدمات المحاماة والاستشارات القانونية.
...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      return new Response(JSON.stringify({ error: "غير مصرح: يجب تسجيل الدخول" }), {
        status: 401,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (rawBody.length > MAX_PAYLOAD_BYTES) {
      return new Response(JSON.stringify({ error: "حجم الطلب يتجاوز الحد المسموح" }), {
        status: 413,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!parsed.success) {
      return new Response(JSON.stringify({ error: "طلب غير صالح", details: parsed.error.issues }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!response.ok) {
      if (response.status === 429)
        return new Response(JSON.stringify({ error: "تجاوزت حد الاستخدام، حاول لاحقاً." }), {
          status: 429,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      if (response.status === 402)
        return new Response(JSON.stringify({ error: "يلزم شحن رصيد المساعد الذكي." }), {
          status: 402,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
