...
import {
  ExternalLink, Briefcase, Wallet, CalendarDays, FileText, User, Archive,
  ArchiveRestore, Trash2, Activity, ScrollText, CalendarPlus,
} from "lucide-react";
...
export function ArchivedClientDetailsModal({ clientId, onClose }: { clientId: string | null; onClose: () => void }) {
  ...
  const totals = useMemo(() => {
    const total = payments.reduce((a: number, p: any) => a + Number(p.total_amount ?? p.amount ?? 0), 0);
    const paid = payments.filter((p: any) => p.status === "paid").reduce((a: number, p: any) => a + Number(p.total_amount ?? p.amount ?? 0), 0);
    return { total, paid, remaining: Math.max(0, total - paid) };
  }, [payments]);

  const activity = useMemo(() => {
    const out: { id: string; date: string; title: string; hint?: string; icon: any }[] = [];
    sessions.forEach((s: any) => out.push({
      id: `s-${s.id}`, date: s.created_at ?? s.session_date, icon: CalendarPlus,
      title: "تمت إضافة جلسة", hint: format(new Date(s.session_date), "yyyy/MM/dd HH:mm"),
    }));
    documents.forEach((d: any) => out.push({
      id: `d-${d.id}`, date: d.created_at, icon: FileText, title: "تم رفع مستند", hint: d.name,
    }));
    payments.forEach((p: any) => out.push({
      id: `p-${p.id}`, date: p.paid_at ?? p.created_at, icon: Wallet,
      title: p.status === "paid" ? "تم تسجيل دفعة مدفوعة" : "تم إضافة دفعة معلّقة",
      hint: `${Number(p.total_amount ?? p.amount ?? 0).toLocaleString()} ج.م`,
    }));
    cases.forEach((c: any) => out.push({
      id: `c-${c.id}`, date: c.created_at, icon: Briefcase, title: "تم فتح القضية", hint: `قضية #${c.case_number}`,
    }));
    if (client?.archived_at) out.push({
      id: "arch", date: client.archived_at, icon: Archive, title: "تمت أرشفة الموكل",
    });
  ...
  const restore = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("clients").update({ is_archived: false, archived_at: null }).eq("id", clientId!);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  return (
    <DetailsModalShell open={open} onClose={onClose} title={title} subtitle={subtitle} actions={actions}>
      {isLoading || !client ? (
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
            ...
            <SummaryCard label="إجمالي الأتعاب" value={`${totals.total.toLocaleString()} ج.م`} small />
            <SummaryCard label="المدفوع" value={`${totals.paid.toLocaleString()} ج.م`} small accent="emerald" />
            <SummaryCard label="المتبقي" value={`${totals.remaining.toLocaleString()} ج.م`} small accent="rose" />
          </div>
          ...
          <Tabs defaultValue="info" className="w-full">
            <TabsList className="w-full overflow-x-auto flex justify-start gap-1 bg-card border border-border p-1 h-auto rounded-xl">
              ...
              <TabsTrigger value="poa" className="text-xs"><ScrollText className="w-3.5 h-3.5 ml-1" />التوكيل</TabsTrigger>
              ...
              <TabsTrigger value="payments" className="text-xs"><Wallet className="w-3.5 h-3.5 ml-1" />المدفوعات</TabsTrigger>
              <TabsTrigger value="documents" className="text-xs"><FileText className="w-3.5 h-3.5 ml-1" />المستندات</TabsTrigger>
              <TabsTrigger value="activity" className="text-xs"><Activity className="w-3.5 h-3.5 ml-1" />السجل</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="payments" className="mt-4">
              {payments.length === 0 ? <Empty msg="لا توجد مدفوعات." /> : (
                <div className="space-y-2">
            ...
            <TabsContent value="activity" className="mt-4">
              {activity.length === 0 ? <Empty msg="لا يوجد نشاط." /> : (
                <ul className="space-y-2">
                  {activity.map((e) => {
                    const Icon = e.icon;
                    return (
                      <li key={e.id} className="flex items-start gap-3 border border-border rounded-md p-3">
                        <Icon className="w-4 h-4 text-[var(--gold)] mt-0.5 shrink-0" />
                        <div className="min-w-0 flex-1">
                          <div className="text-sm font-medium">{e.title}</div>
                          {e.hint && <div className="text-xs text-muted-foreground truncate">{e.hint}</div>}
                        </div>
                        <div className="text-[11px] text-muted-foreground shrink-0">{format(new Date(e.date), "yyyy/MM/dd")}</div>
                      </li>
                    );
                  })}
                </ul>
          ...
          <div className="sticky bottom-0 -mx-4 md:-mx-6 -mb-4 md:-mb-6 mt-4 px-4 md:px-6 py-3 bg-background/80 backdrop-blur border-t border-border flex flex-wrap gap-2 justify-end">
            <Link to="/clients/$id" params={{ id: clientId! }} onClick={onClose} className="md:hidden inline-flex items-center gap-1.5 text-xs px-3 py-2 rounded-md border border-border">
            ...
            >
              <ArchiveRestore className="w-3.5 h-3.5" /> {restore.isPending ? "..." : "استعادة الموكل"}
            </button>
            {isAdmin && (
              <button
      ...
      {confirmHardDel && (
        <div className="fixed inset-0 z-[70] grid place-items-center bg-black/70 backdrop-blur-sm p-4" onClick={() => setConfirmHardDel(null)}>
          <div className="glass-card w-full max-w-md p-6 border-rose-500/40" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2 justify-end">
              <button onClick={() => setConfirmHardDel(null)} className="px-4 py-2 rounded-md border border-border text-sm">إلغاء</button>
              {confirmHardDel === 1 ? (
                <button onClick={() => setConfirmHardDel(2)} className="px-4 py-2 rounded-md bg-rose-500/80 text-white text-sm font-semibold">متابعة</button>
              ) : (
                <button disabled={hardDelete.isPending} onClick={() => hardDelete.mutate()} className="px-4 py-2 rounded-md bg-rose-600 text-white text-sm font-semibold">
...
function SummaryCard({ label, value, small, accent }: { label: string; value: React.ReactNode; small?: boolean; accent?: "emerald" | "rose" }) {
  const color = accent === "emerald" ? "text-emerald-400" : accent === "rose" ? "text-rose-400" : "text-[var(--gold)]";
  return (
...
function ReadOnlyGrid({ fields, data }: { fields: { key: string; label: string; full?: boolean }[]; data: any }) {
  return (
    <div className="grid sm:grid-cols-2 gap-3">
      {fields.map((f) => (
