...
type Event = { id: string; date: string; type: "session" | "doc" | "payment" | "case"; title: string; hint?: string };
  ...
  const { data: events = [] } = useQuery({
    ...
    queryFn: async (): Promise<Event[]> => {
      const [sess, docs, pay, cs] = await Promise.all([
        supabase.from("sessions").select("id, session_date, notes, created_at").eq("case_id", caseId),
        supabase.from("documents").select("id, name, created_at").eq("case_id", caseId),
        supabase.from("payments").select("id, amount, status, created_at, paid_at").eq("case_id", caseId),
        supabase.from("cases").select("created_at, updated_at, status").eq("id", caseId).maybeSingle(),
      ]);

      const out: Event[] = [];
      (sess.data ?? []).forEach((s: any) => out.push({
        id: `s-${s.id}`, date: s.created_at, type: "session",
        title: "تمت إضافة جلسة", hint: format(new Date(s.session_date), "yyyy/MM/dd HH:mm"),
      }));
      (docs.data ?? []).forEach((d: any) => out.push({
        id: `d-${d.id}`, date: d.created_at, type: "doc",
        title: "تم رفع مستند", hint: d.name,
      }));
      (pay.data ?? []).forEach((p: any) => out.push({
        id: `p-${p.id}`, date: p.paid_at ?? p.created_at, type: "payment",
        title: p.status === "paid" ? "تم تسجيل دفعة مدفوعة" : "تم إضافة دفعة معلّقة",
        hint: `${Number(p.amount).toLocaleString()} ج.م`,
      }));
      if (cs.data) {
        out.push({ id: `c-open-${caseId}`, date: cs.data.created_at, type: "case", title: "تم فتح القضية" });
        if (cs.data.status === "closed") {
          out.push({ id: `c-close-${caseId}`, date: cs.data.updated_at, type: "case", title: "تم إغلاق القضية" });
        }
      }
      ...
      {events.map((e) => {
        ...
        return (
          <li key={e?.id} className="pr-6 relative">
            ...
            <div className="text-[10px] text-muted-foreground mt-0.5">{format(new Date(e.date), "yyyy/MM/dd HH:mm")}</div>
          </li>
        );
      })}
    </ol>
