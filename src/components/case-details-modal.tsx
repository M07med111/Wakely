...
export function CaseDetailsModal({
  ...
  const editFields = [
    ...
    { key: "charge", label: "التهمة" },
    { key: "case_year", label: "السنة", type: "number" },
    { key: "case_category", label: "نوع القضية", select: CASE_CATEGORIES },
    ...(form.case_category === "مدني"
      ? [{ key: "case_sub_type", label: "تصنيف القضية", select: ["استئناف عالي مدني", "مدني جزئي", "مدني مستأنف", "صحة وتوثيق"] }]
      : form.case_category === "جنائي"
        ? [{ key: "case_sub_type", label: "تصنيف القضية", select: ["جنح", "إداري", "جنح مرور", "جنح اقتصادية"] }]
        : []),
    ...
    { key: "session_type", label: "نوع الجلسة", select: ["أسرة", "مدني", "جنائي", "استئناف عالي", "أول درجة", "معارضة", "استئناف", "معارضة استئنافية", "نقض", "إشكال", "تحقيق", "تجديد"] },
                    ...
                    f.select ? (
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          if (f.key === "case_category") newForm.case_sub_type = "";
                          setForm(newForm);
                        }}
