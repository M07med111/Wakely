...
import { Pencil, Save, X as XIcon, ExternalLink, Briefcase, Wallet, CalendarDays, FileText } from "lucide-react";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { StatusBadge } from "@/components/status-badge";
import { DetailsModalShell } from "@/components/details-modal-shell";
import { CASE_CATEGORIES, formatCaseId } from "@/components/new-case-modal";
...
export function CaseDetailsModal({ caseId, onClose }: { caseId: string | null; onClose: () => void }) {
  ...
  const { data: c, isLoading } = useQuery({
    ...
    enabled: !!caseId,
    queryFn: async () => {
      const { data, error } = await supabase.from("cases").select("*, clients(id, full_name, phone)").eq("id", caseId!).maybeSingle();
  ...
  const { data: sessions = [] } = useQuery({
    ...
    enabled: !!caseId,
    queryFn: async () => (await supabase.from("sessions").select("*").eq("case_id", caseId!).order("session_date", { ascending: false })).data ?? [],
  });
  const { data: payments = [] } = useQuery({
    queryKey: ["case-payments", caseId],
    enabled: !!caseId,
    queryFn: async () => (await supabase.from("payments").select("*").eq("case_id", caseId!).order("created_at", { ascending: false })).data ?? [],
  });
  const { data: documents = [] } = useQuery({
    queryKey: ["case-documents", caseId],
    enabled: !!caseId,
    queryFn: async () => (await supabase.from("documents").select("*").eq("case_id", caseId!).order("created_at", { ascending: false })).data ?? [],
  });

  useEffect(() => {
    if (c) {
      const f: any = { ...c };
      if (f.next_session_date) f.next_session_date = new Date(f.next_session_date).toISOString().slice(0, 16);
      setForm(f);
    }
    setEditing(false);
  }, [c]);
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (payload.next_session_date) {
        ...
        const exists = sessions.some((s: any) => new Date(s.session_date).toISOString() === newDateIso);
        if (!exists) {
          const { data: u } = await supabase.auth.getUser();
          if (u.user?.id) {
            await supabase.from("sessions").insert({
              user_id: u.user.id, case_id: caseId!, session_date: newDateIso,
  ...
  const actions = c && (
    <>
      <Link to="/cases/$id" params={{ id: caseId! }} onClick={onClose} className="hidden md:inline-flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-md border border-border hover:border-[var(--gold)]">
      ...
      {!editing ? (
        ...
        <>
          ...
          <button onClick={() => { setForm(c); setEditing(false); }} className="inline-flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-md border border-border">
      ...
      {isLoading || !c ? (
        ...
        <Tabs defaultValue="info" className="w-full">
          <TabsList className="w-full overflow-x-auto flex justify-start gap-1 bg-card border border-border p-1 h-auto rounded-xl">
            <TabsTrigger value="info" className="text-xs"><Briefcase className="w-3.5 h-3.5 ml-1" />البيانات</TabsTrigger>
            <TabsTrigger value="sessions" className="text-xs"><CalendarDays className="w-3.5 h-3.5 ml-1" />الجلسات ({sessions.length})</TabsTrigger>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {EDIT_FIELDS.map((f) => (
                <div key={f.key} className={f.full ? "sm:col-span-2" : ""}>
                  ...
                  {editing ? (
                    f.select ? (
                      <select value={form[f.key] ?? ""} onChange={(e) => setForm({ ...form, [f.key]: e.target.value })} className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm outline-none focus:border-[var(--gold)]">
                        {f.select.map((o) => <option key={o} value={o}>{o === "active" ? "متداولة" : o === "closed" ? "منتهية" : o}</option>)}
                      </select>
                    ) : f.full ? (
                      <textarea rows={3} value={form[f.key] ?? ""} onChange={(e) => setForm({ ...form, [f.key]: e.target.value })} className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm outline-none focus:border-[var(--gold)]" />
                    ) : (
                      <input type={f.type ?? "text"} value={form[f.key] ?? ""} onChange={(e) => setForm({ ...form, [f.key]: e.target.value })} className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm outline-none focus:border-[var(--gold)]" />
                    )
                  ) : (
                    <div className="text-sm font-medium bg-card/50 border border-border rounded-md px-3 py-2 min-h-[2.25rem]">
                      {f.key === "next_session_date" && (c as any)[f.key]
                        ...
                        : f.key === "status"
                        ? ((c as any)[f.key] === "closed" ? "منتهية" : "متداولة")
                        : (c as any)[f.key] || "—"}
                    </div>
                  )}
                </div>
