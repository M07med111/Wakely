  ...
  const {
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("payments")
        .select("*, clients(id, full_name), cases(id, case_number)")
    ...
    <div>
      ...
      <div className="grid sm:grid-cols-2 gap-4 mb-6">
        <div className="glass-card p-5">
          ...
          <div className="text-3xl font-bold gold-text mt-2">{totalPaid.toLocaleString()} ج.م</div>
        </div>
        <div className="glass-card p-5">
          <div className="text-xs text-muted-foreground">إجمالي المعلّق</div>
      ...
      {payments.length === 0 ? (
        <EmptyState
          ...
          description="سجّل أتعاب القضايا والمدفوعات هنا."
            ...
            {payments
              ...
              .map((p: any) => {
                const overdue =
                  p.status === "pending" &&
                  p.due_date &&
                  differenceInDays(new Date(), new Date(p.due_date)) > 0;
                return (
                  <div
                    key={p?.id}
                    className={`glass-card p-4 ${overdue ? "border-rose-500/40" : ""}`}
                  >
                    <div className="flex items-center justify-between mb-1">
                      {p.clients?.id ? (
                        <Link
                          ...
                          className="font-semibold text-sm truncate hover:text-[var(--gold)]"
                        >
                          {p.clients.full_name}
                        </Link>
                      ) : (
                      ...
                      <StatusBadge
                        status={overdue ? "overdue" : p.status === "paid" ? "paid" : "pending"}
                      />
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <button
                        ...
                        className="text-xs text-[var(--gold)] flex items-center gap-1"
                      >
                        <CheckCircle2 className="w-3.5 h-3.5" />
                        {p.status === "paid" ? "تراجع" : "تحديد كمدفوع"}
                      </button>
                    </div>
                  </div>
                );
