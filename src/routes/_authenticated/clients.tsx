...
import { Plus, UserCircle2, Phone, X, Users, Mail, FileText, Archive, Trash2 } from "lucide-react";
...
function ClientsPage() {
  ...
  const deleteClient = useMutation({
    ...
    onSuccess: () => {
      ...
      if (detailsClientId === confirmDelete?.id) setDetailsClientId(null);
  ...
  const create = useMutation({
    mutationFn: async (form: any) => {
      ...
      if (form._poaFile) {
        ...
        const path = createStoragePath([userId, "poa"], f);
      ...
      const payload: any = {
        ...
        notarization_office: form.notarization_office || null,
        poa_file_path,
      };
      const { data, error } = await supabase
        ...
        .maybeSingle();
      if (error) throw error;
      if (poaFile && poa_file_path && data?.id) {
        const { error: docErr } = await supabase.from("documents").insert({
          user_id: userId,
          client_id: data.id,
          name: poaFile.name,
          storage_path: poa_file_path,
          mime_type: poaFile.type,
          size_bytes: poaFile.size,
        });
          ...
          .map((c, i) => (
            ...
            >
              ...
              <button
                ...
                title="أرشفة الموكل"
                className="p-1.5 rounded-md border border-border bg-card/80 backdrop-blur hover:border-[var(--gold)] text-muted-foreground hover:text-[var(--gold)]"
              >
              ...
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setConfirmDelete({ id: c.id, name: c.full_name, step: 1 });
                }}
                title="حذف الموكل"
                className="p-1.5 rounded-md border border-rose-500/40 bg-card/80 backdrop-blur text-rose-400 hover:bg-rose-500/10"
              >
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 backdrop-blur-sm p-4">
          <div className="glass-card w-full max-w-md p-6 border-rose-500/40">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="text-lg font-bold text-rose-400">حذف الموكل</h3>
            </div>
            <p className="text-sm text-muted-foreground mb-4">
              سيتم حذف <strong className="text-foreground">{confirmDelete.name}</strong> نهائيا،
              مع البيانات المرتبطة به. لا يمكن التراجع عن هذا الاجراء.
            </p>
            {confirmDelete.step === 2 && (
              <div className="mb-4 p-3 rounded-md bg-rose-500/10 border border-rose-500/40 text-sm text-rose-300">
                تأكيد أخير قبل الحذف النهائي.
              </div>
            )}
            <div className="flex gap-2 justify-end">
              <button
        ...
        >
          ...
          <div className="border-t border-border pt-3 mt-3">
            ...
            <div className="grid grid-cols-2 gap-3 mt-3">
              ...
              <Field label="مكتب التوثيق">
                <input
                  value={form.notarization_office}
                  onChange={(e) => setForm({ ...form, notarization_office: e.target.value })}
                  className="cinput"
                />
              </Field>
            </div>
