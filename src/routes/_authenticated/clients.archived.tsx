...
function ArchivedClientsPage() {
  ...
  const [confirmHardDel, setConfirmHardDel] = useState<{ id: string; name: string; step: 1 | 2 } | null>(null);
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        <p className="text-muted-foreground mt-1">يمكنك استعادة أي موكل لإعادته للقوائم الرئيسية.</p>
      </div>

      {isLoading ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 animate-pulse">
          {[0, 1, 2].map((i) => <div key={i} className="h-32 bg-muted/50 rounded-xl" />)}
        </div>
      ) : clients.length === 0 ? (
        <EmptyState icon={Archive} title="لا يوجد موكلون مؤرشفون" description="جميع موكليك نشطون حالياً." />
      ) : (
          ...
          {clients.filter((c: any) => c?.id).map((c: any) => (
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground border border-border">مؤرشف</span>
              </div>
              {c.archived_at && (
                <div className="text-[11px] text-muted-foreground mb-3">أُرشف في: {format(new Date(c.archived_at), "yyyy/MM/dd")}</div>
              )}
              <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
                ...
                {isAdmin && (
                  <button
                    onClick={(e) => { e.stopPropagation(); setConfirmHardDel({ id: c.id, name: c.full_name, step: 1 }); }}
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 backdrop-blur-sm p-4">
          <div className="glass-card w-full max-w-md p-6 border-rose-500/40">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              سيتم حذف الموكل <strong className="text-foreground">{confirmHardDel.name}</strong> وجميع القضايا والجلسات والمدفوعات والمستندات نهائياً ولا يمكن التراجع عن هذه العملية.
            </p>
            ...
            <div className="flex gap-2 justify-end">
              ...
              {confirmHardDel.step === 1 ? (
                <button onClick={() => setConfirmHardDel({ ...confirmHardDel, step: 2 })} className="px-4 py-2 rounded-md bg-rose-500/80 text-white text-sm font-semibold">
                  متابعة
                </button>
              ) : (
                <button
