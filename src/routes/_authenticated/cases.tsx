...
import { NewCaseModal, formatCaseId } from "@/components/new-case-modal";
...
function CasesPage() {
  ...
  const { data: cases = [], isLoading, error } = useQuery({
    queryKey: ["cases", q],
    queryFn: async () => {
      let query = supabase.from("cases").select("*, clients(id, full_name)").order("created_at", { ascending: false });
  ...
  return (
    <div>
      ...
      <input placeholder="ابحث برقم القضية..." value={q} onChange={(e) => setQ(e.target.value)}
      ...
      {cases.length === 0 ? (
        <EmptyState icon={Briefcase} title="لا توجد قضايا بعد" description="ابدأ بفتح أول قضية لمتابعتها." actionLabel="قضية جديدة" onAction={() => setOpen(true)} />
      ) : (
        <>
          {/* Mobile: cards */}
          <div className="md:hidden space-y-2">
            {cases.filter((c: any) => c?.id).map((c: any) => (
              <button type="button" key={c?.id} onClick={() => setDetailsCaseId(c?.id)} className="glass-card p-4 block w-full text-right active:scale-[0.99]">
                <div className="flex items-center justify-between mb-2 gap-2">
                  <span className="font-bold text-[var(--gold)] text-xs truncate">{formatCaseId(c)}</span>
                ...
                {c.clients?.id ? (
                  <span onClick={(e) => { e.preventDefault(); e.stopPropagation(); setDetailsClientId(c.clients.id); }} className="text-sm font-semibold truncate text-right hover:text-[var(--gold)] cursor-pointer block">{c.clients?.full_name ?? "—"}</span>
                ) : (
                  <div className="text-sm font-semibold truncate">{c.clients?.full_name ?? "—"}</div>
                )}
                <div className="text-[11px] text-muted-foreground mt-1 truncate">
                  {c.court_name ?? c.court_location ?? "—"} {c.opponent_name && `· الخصم: ${c.opponent_name}`}
                </div>
              </button>
            ))}
          </div>
          ...
          <div className="glass-card overflow-hidden hidden md:block">
            <table className="w-full text-sm">
              ...
              <tbody>
                {cases.filter((c: any) => c?.id).map((c: any) => (
                  <tr key={c?.id} onClick={() => setDetailsCaseId(c?.id)} className="border-t border-border hover:bg-muted/30 cursor-pointer">
                    <td className="p-3">
                      <span className="text-[var(--gold)] font-semibold block">{formatCaseId(c)}</span>
                    </td>
                    <td className="p-3">
                      {c.clients?.id ? (
                        <span onClick={(e) => { e.stopPropagation(); setDetailsClientId(c.clients.id); }} className="hover:text-[var(--gold)] cursor-pointer">{c.clients.full_name}</span>
                      ) : (c.clients?.full_name ?? "—")}
                    </td>
...
function PageLoading() {
  return <div className="p-6 space-y-3 animate-pulse"><div className="h-8 w-40 bg-muted rounded" /><div className="h-12 w-full bg-muted/50 rounded" /><div className="h-52 bg-muted/40 rounded-xl" /></div>;
}
