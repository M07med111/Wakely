...
import { CASE_CATEGORIES } from "@/lib/case-format";

export const Route = createFileRoute("/_authenticated/cases_/$id")({
          ...
          <div className="glass-card p-6">
            {editing ? (
              <div className="grid sm:grid-cols-2 gap-4 text-sm">
                ...
                />
                <div>
                  <div className="text-xs text-muted-foreground mb-1">نوع القضية</div>
                  <select
                    value={form.case_category ?? ""}
                    onChange={(e) => setForm({ ...form, case_category: e.target.value, case_sub_type: "" })}
                ...
                {form.case_category === "مدني" && (
                  <div>
                    <div className="text-xs text-muted-foreground mb-1">تصنيف القضية</div>
                    <select
                      value={form.case_sub_type ?? ""}
                      onChange={(e) => setForm({ ...form, case_sub_type: e.target.value })}
                      className="w-full bg-input border border-border rounded-md px-3 py-2 outline-none focus:border-[var(--gold)] text-sm"
                    >
                      <option value="">اختر التصنيف</option>
                      <option value="استئناف عالي مدني">استئناف عالي مدني</option>
                      <option value="مدني جزئي">مدني جزئي</option>
                      <option value="مدني مستأنف">مدني مستأنف</option>
                      <option value="صحة وتوثيق">صحة وتوثيق</option>
                    </select>
                  </div>
                )}
                {form.case_category === "جنائي" && (
                  <div>
                    <div className="text-xs text-muted-foreground mb-1">تصنيف القضية</div>
                    <select
                      value={form.case_sub_type ?? ""}
                ...
                <div className="sm:col-span-2">
                  ...
                  <input
                    ...
                    className="w-full bg-input border border-border rounded-md px-3 py-2 outline-none focus:border-[var(--gold)] text-sm mb-3"
                  />
                  <div className="text-xs text-muted-foreground mb-1">نوع الجلسة</div>
                  <select
                  ...
                  >
                    ...
                    <option value="استئناف عالي">استئناف عالي</option>
                    <option value="أول درجة">أول درجة</option>
                    <option value="معارضة">معارضة</option>
                    <option value="استئناف">استئناف</option>
                    <option value="معارضة استئنافية">معارضة استئنافية</option>
                    <option value="نقض">نقض</option>
                    <option value="إشكال">إشكال</option>
              ...
              <>
                <div className="grid sm:grid-cols-3 gap-4 text-sm">
                  ...
                  <Info label="نوع القضية" value={c.case_category} />
                  {c.case_sub_type && <Info label="تصنيف القضية" value={c.case_sub_type} />}
                  {c.charge && <Info label="التهمة" value={c.charge} />}
                  <Info
                ...
                .map((s: any) => (
                  <div key={s?.id} className="border-r-2 border-[var(--gold)] pr-4 py-2">
                    <div className="font-semibold text-sm flex items-center gap-2">
                      ...
                      {s.session_type && (
                        <span className="bg-[var(--gold)]/10 text-[var(--gold)] px-2 py-0.5 rounded-full text-xs">
                          {s.session_type}
                        </span>
                      )}
                    </div>
